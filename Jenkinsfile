...
"
        ...
        // STAGE 2.5: SECURITY SCANNING (Trivy) — every branch, gates the PR
        stage('Security Scans') {
            steps {
                echo '🛡️  Building images for scanning...'
                sh 'docker build -t ${APP_NAME}-backend:scan-${BUILD_NUMBER} ./backend'
                sh 'docker build -t ${APP_NAME}-frontend:scan-${BUILD_NUMBER} ./frontend'
                ...
                sh '''
                    docker run --rm \
                      ...
                      ${APP_NAME}-backend:scan-${BUILD_NUMBER}
                '''
                ...
                always {
                    ...
                    sh '''
                        docker run --rm \
                          ...
                          ${APP_NAME}-backend:scan-${BUILD_NUMBER} > trivy-backend-report.txt 2>&1 || true
                    '''
                    sh '''
                        docker run --rm \
                          -v /var/run/docker.sock:/var/run/docker.sock \
                          -v trivy-cache:/root/.cache/trivy \
                          aquasec/trivy:latest image --exit-code 0 --no-progress \
                          ${APP_NAME}-frontend:scan-${BUILD_NUMBER} > trivy-frontend-report.txt 2>&1 || true
                    '''
                    archiveArtifacts artifacts: 'trivy-*-report.txt', allowEmptyArchive: true

                    echo '🧹 Cleaning up scan images...'
                    sh 'docker rmi ${APP_NAME}-backend:scan-${BUILD_NUMBER} ${APP_NAME}-frontend:scan-${BUILD_NUMBER} || true'
                }
            }
        }
